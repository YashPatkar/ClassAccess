...
def home(request):
    if request.method == 'POST':
        ...
        print("___OBJECT CREATED___")

        request.session['user'] = {'time', dt.datetime.now()}
        code_entry = pdfcode.objects.get(code=code)
        
        return render(request, 'app1/home.html', \
                      {'check': "created", 'code': code_entry.code})
    return render(request, 'app1/home.html', {'check': "notcreated"})

def viewpdf(request, code):
    if request.method == "POST" and os.path.exists(f'media/{code}.pdf'):
        try:
            pdf_entry = pdfpath.objects.get(code=code)
